...
| **测试** | 22 个测试程序、1491 项检查，`ctest` 一键跑完。其中 `test_endtoend` 是**全链路**：把"开机到关机"会经过的所有模块串起来跑一遍（会话恢复 → 新建/输入 → 渲染与行号映射 → 保存 → 多标签 → 全文搜索与跳转 → 导出 HTML → 主题切换 → 关闭再恢复 → 1MB 大文档 → 外部修改与重载） |
| **性能** | 预览防抖 300ms（连打十个字只渲染一次）；**大文档快速模式**：超过 30 万字符自动关掉语法高亮并暂停预览，保证 1MB 级文件编辑流畅；**启动两阶段**：先显示窗口，再把文件树监听与会话恢复排到下一轮；**内存**：缓存是每个文档自己的（关标签即释放），单文档上限 20 条 / 单条 1MB |
