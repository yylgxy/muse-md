...
public:
    ...
    // 这段**已经解码好的文本**看起来是不是二进制文件（.pdb / .ilk / .docx / .exe …）？
    //
    // 判据：出现 NUL 字符（U+0000），或者控制字符（除 \t \r \n \f 之外）占比超过 10%
    // 且至少有 16 个（少量控制字符可能只是老文件用 ^Z 结尾之类，不能一棍子打死）。
    //
    // 为什么在"解码之后"判断，而不是直接看原始字节：
    // UTF-16 文本的原始字节里**本来就有大量 0x00**（每个 ASCII 字符后面跟一个 0），
    // 直接按字节判会把正常的 UTF-16 文档判成二进制。按编码解码之后，
    // UTF-16 文本里的 NUL 字节已经配对成字符了，于是：
    //   * 真正的 UTF-16 / GBK / UTF-8 文本 → 样本里没有 U+0000，控制字符也极少 → 不是二进制；
    //   * 二进制文件（不管按哪种编码解）→ 剩下大量 NUL / 控制字符 → 是二进制。
